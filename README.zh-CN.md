...
| 📤 发送名片文件 | 一键把 `.vcf` 名片文件通过分享面板发出（AirDrop / 微信 / 邮件…），对方点开即存入通讯录 |
...
| 隔空投递给附近的 iPhone / Mac | 点**发送名片文件** → 选 AirDrop → 对方收到 `.vcf` 直接存入通讯录 |
| 聊天软件（微信 / WhatsApp / 短信…） | **分享链接**，或把**名片文件**当附件发过去 |
...
A：点名片上的**发送名片文件** —— 系统分享面板里就有 AirDrop。对方收到 `.vcf` 文件，点开直接存入通讯录。（不支持的浏览器会自动回退为下载文件。）
